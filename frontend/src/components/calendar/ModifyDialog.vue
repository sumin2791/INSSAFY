        ...
        <v-form class="px-3" ref="form">
          ...
          <div class="text-center" style="margin-bottom: 10px;">
            ...
            <v-btn text class="primary white--text mx-2 mt-3 bottom-btn newmorphism m-desc" @click="submit">
              <span style="color:var(--basic-color-key);">수정</span>
            </v-btn>
          </div>
        </v-form>
      </v-card-text>
...
export default {
  ...
  computed: {
    ...mapState('calendar', ['modifyDialog', 'event']),
    // titleRule() {
  ...
  watch: {
    ...
    modifyDialog(v) {
      this.dialogState = v;
      if (this.event.hasTime) {
        this.event.startTime = this.event.startTime.slice(0, 5);
        this.event.endTime = this.event.endTime.slice(0, 5);
      }
    },
  },
  methods: {
    submit() {
      if (this.event.name === '' || this.event.endDate === '') {
        ...
        this.$store.dispatch('calendar/actModifyEvent', { event: this.event, boardName: this.boardName }).then((result) => {
          if (result) {
            this.$toast.open({
              message: '일정이 수정되었습니다.',
              type: 'success',
    ...
    close() {
      this.$store.commit('calendar/CLOSE_MODIFY_CALENDAR_DIALOG');
    },
    selectTime() {
